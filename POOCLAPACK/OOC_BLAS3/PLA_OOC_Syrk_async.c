		   ...
		   PLA_Obj beta,  PLA_Obj C )
{
  ...
  while ( TRUE ){
    ...
    if ( ( size = min( size, nb_ooc ) ) != 0 ) {
      if ( transa == PLA_NO_TRANS ) 
	PLA_Obj_vert_split_2( A_ooc_L, size, &A_ooc_2, &A_ooc_L );
      else {
	PLA_Obj_horz_split_2( A_ooc_L, size, &A_ooc_2, 
                                              &A_ooc_L );
      }
/*
      if(!first_time)
...
//      PLA_OOC_Copy_wait( A_ooc_1);
  ...
  PLA_Obj_free( &A_ooc_2);   PLA_Obj_free( &A_in_2 );
