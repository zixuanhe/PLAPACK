      SUBROUTINE SSTEQR_X( COMPZ, NROWS_Z, N, D, E, Z, LDZ, WORK, INFO )
*
...
*
      IF( ICOMPZ.EQ.2 )
     $   CALL SLASET( 'Full', NROWS_Z, N, ZERO, ONE, Z, LDZ )
*
      NMAXIT = N*MAXIT
      JTOT = 0
*
...
*
         IF( M.EQ.L+1 ) THEN
            IF( ICOMPZ.GT.0 ) THEN
               ...
               WORK( N-1+L ) = S
               if ( NROWS_Z .GT. 0 )
     $              CALL SLASR( 'R', 'V', 'B', NROWS_Z, 2, WORK( L ),
...
*
         IF( ICOMPZ.GT.0 ) THEN
            MM = M - L + 1
               if ( NROWS_Z .GT. 0 )
     $           CALL SLASR( 'R', 'V', 'B', NROWS_Z, MM, WORK( L ), 
     $                  WORK( N-1+L ),
     $                  Z( 1, L ), LDZ )
         END IF
*
...
*
         IF( M.EQ.L-1 ) THEN
            IF( ICOMPZ.GT.0 ) THEN
               ...
               WORK( N-1+M ) = S
               if ( NROWS_Z .GT. 0 )
     $              CALL SLASR( 'R', 'V', 'F', NROWS_Z, 2, WORK( M ),
...
*
         IF( ICOMPZ.GT.0 ) THEN
            MM = L - M + 1
            if ( NROWS_Z .GT. 0 )
     $           CALL SLASR( 'R', 'V', 'F', NROWS_Z, MM, WORK( M ), 
     $                  WORK( N-1+M ),
     $                  Z( 1, M ), LDZ )
         END IF
*
...
*
  ...
  170       CONTINUE
            IF( K.NE.I ) THEN
               ...
               D( I ) = P
               if ( NROWS_Z .GT. 0 )
     $              CALL SSWAP( NROWS_Z, Z( 1, I ), 1, Z( 1, K ), 1 )
            END IF
  180    CONTINUE
      END IF
*
