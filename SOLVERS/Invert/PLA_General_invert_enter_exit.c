...
int PLA_General_invert_enter( int method, PLA_Obj A )
{
  int 
    value = PLA_SUCCESS,
    length_A, width_A, 
  ...
  char 
    routine_name[ 35 ] = "PLA_General_invert";

  PLA_Routine_stack_push( routine_name );

  PLA_Routine_stack_push( "PLA_General_invert_enter" );
  ...
  if ( PLA_CHECK_PARAMETERS ){
    ...
    if ( method != PLA_METHOD_INV && method != PLA_METHOD_FACTORS 
	 && method != PLA_METHOD_STABLE && method != PLA_METHOD_TRSM ) {
      PLA_Warning( "Unknown method" );
      value--;
    }
...
int PLA_General_invert_exit( int method, PLA_Obj A )

{
  int value = PLA_SUCCESS,
      size_malloced;
  double 
    *norm_A_p, *norm_residual_p;
  char 
    routine_name[ 35 ];
  PLA_Obj
    residual = NULL, norm_A = NULL, norm_residual = NULL,
    one = NULL, minus_one = NULL;


  PLA_Routine_stack_push( "PLA_General_invert_exit" );
  ...
  if ( PLA_CHECK_AGAINST_SEQUENTIAL ){
    ...
    PLA_Gemm( PLA_NO_TRANS, PLA_NO_TRANS, minus_one, A, A_cpy, one, 
	       residual );
	...
	PLA_Warning( "PLA_General_invert: large relative error encountered" );
  ...
  if ( size_malloced != old_size_malloced )
    PLA_Warning( "PLA_General_invert: memory discrepency" );
