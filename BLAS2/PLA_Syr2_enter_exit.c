                     ...
                     PLA_Obj A )
{
  int
    ...
    objtype, proj_onto;
  char 
    routine_name[ 35 ] = "PLA_Syr2";

  PLA_Routine_stack_push( routine_name );

  PLA_Routine_stack_push( "PLA_Syr2_enter" );
  ...
  if ( PLA_CHECK_PARAMETERS ){
   ...
   PLA_Obj_objtype( alpha, &objtype );
    ...
    if ( length_A != length_x ){
      PLA_Warning( "dimension of A does not match length of x" );
      value--;
    }      
    if ( length_A != length_y ){
      PLA_Warning( "dimension of A does not match length of y" );
      value--;
    }      
  }
...

{
  ...
  PLA_Routine_stack_push( "PLA_Syr2_exit" );
  ...
  if ( PLA_CHECK_AGAINST_SEQUENTIAL ){
    ...
    max_A = PLA_Local_abs_max( A_temp );
    max_x = PLA_Local_abs_max( x_cpy );
    max_y = PLA_Local_abs_max( y_cpy );

    max_all = max( max_A, max( max_x, max_y ) ); 

    /*    PLA_Global_show( "x = ", x_cpy, "%lf ", " " );
    PLA_Global_show( "y = ", y_cpy, "%lf ", " " );
    PLA_Global_show( "A = ", A_cpy, "%lf ", " " );
    */

    PLA_Local_syr2( uplo, alpha_cpy, x_cpy, y_cpy, A_cpy );

    /*
    PLA_Global_show( "par A = ", A, "%lf ", " " );
    PLA_Global_show( "seq A = ", A_cpy, "%lf ", " " );
    */

    diff = PLA_Local_abs_diff( A_temp, A_cpy );

    if ( diff > 0.000001 * max_all ){
      PLA_Warning( "PLA_Syr2: large relative error encountered" );
      value--;
    }      
  ...
  if ( size_malloced != old_size_malloced )
    PLA_Warning( "PLA_Syr2: memory discrepency" );
