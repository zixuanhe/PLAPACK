...
int PLA_Asum_enter( PLA_Obj x, PLA_Obj alpha )
{
  int
    value = PLA_SUCCESS,
    ...
    objtype;
  char 
    routine_name[ 35 ] = "PLA_Asum";
  MPI_Datatype
    datatype;

  PLA_Routine_stack_push( routine_name );

  PLA_Routine_stack_push( "PLA_Asum_enter" );
...
int PLA_Asum_exit( PLA_Obj x, PLA_Obj alpha )
{
  ...
  PLA_Routine_stack_push( "PLA_Asum_exit" );

  if ( PLA_CHECK_AGAINST_SEQUENTIAL ){
    PLA_Local_asum( x_cpy, alpha_cpy );
  ...
  if ( size_malloced != old_size_malloced )
    PLA_Warning( "PLA_Asum: memory discrepency" );
