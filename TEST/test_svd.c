...
int main(int argc, char *argv[])
{
  ...
  PLA_Template 
    templ = NULL;
  PLA_Obj  
    A_orig  = NULL, A      = NULL, A_diag = NULL, U = NULL,  V = NULL, diag = NULL,  B = NULL,
    minus_one = NULL, zero   = NULL, one  = NULL, sL = NULL, sR = NULL, D = NULL;
  int      
    m, n,
    nb_distr, nb_alg,
...
/*  PLA_Comm_1D_to_2D_ratio(MPI_COMM_WORLD, 1.0, &comm); */
  ...
  while ( TRUE ){
    ...
    PLA_Create_svd_problem( 3, diag, A_orig, U, V );

    PLA_Obj_set_to_zero( D );
    ...
    PLA_Obj_split_4( A, min( m, n ), min( m, n ), &A_diag,   PLA_DUMMY, 
		                                  PLA_DUMMY, PLA_DUMMY );

    PLA_Obj_set_to_zero(  A );
    PLA_Obj_set_diagonal( A_diag, D );

    /* A_orig = A_orig - U diag V^T */
    PLA_Gemm( PLA_NO_TRANSPOSE, PLA_NO_TRANSPOSE,
	       one, U, A, zero, B );

    PLA_Gemm( PLA_NO_TRANSPOSE, PLA_TRANSPOSE,
	       minus_one, B, V, one, A_orig );
