...
int PLA_Gemm_x_enter( int version, int transa, int transb, 
		      ...
		      PLA_Obj beta,  PLA_Obj C )
{
  int
    ...
    objtype;
  char 
    routine_name[ 35 ] = "PLA_Gemm_x";

  PLA_Routine_stack_push( routine_name );

  PLA_Routine_stack_push( "PLA_Gemm_x_enter" );
  ...
  if ( PLA_CHECK_PARAMETERS ){
    ...
    if ( !PLA_Valid_trans_parameter( transb ) ){
      PLA_Warning( "Invalid parameter transb" );
      value--;
    }
   ...
   PLA_Obj_objtype( alpha, &objtype );
	...
	value--;
      ...
      if ( transb == PLA_NO_TRANS || transb == PLA_CONJ ){
	if ( width_A != length_B ){
	  PLA_Warning( "width of A does not match length of B" );
	  value--;
	}      
      }
      else {
	...
	value--;
      ...
      if ( width_C != length_B ){
	PLA_Warning( "length of B does not match width of C" );
	value--;
      }       
   }
  }
...
int PLA_Gemm_x_exit( int version, int transa, int transb, 
	             PLA_Obj alpha, PLA_Obj A, PLA_Obj B, 
                     PLA_Obj beta,  PLA_Obj C )

{
  ...
  PLA_Routine_stack_push( "PLA_Gemm_x_exit" );
  ...
  if ( PLA_CHECK_AGAINST_SEQUENTIAL ){
    ...
    PLA_Local_gemm( transa, transb, alpha_cpy, A_cpy, B_cpy,
		                     one,  C_cpy );

    diff = PLA_Local_abs_diff( C_temp, C_cpy );

    if ( diff > 0.000001 * max_all ){
      PLA_Warning( "PLA_Gemm: large relative error encountered" );
      value--;
    }      
  ...
  if ( size_malloced != old_size_malloced )
    PLA_Warning( "PLA_Gemm: memory discrepency" );
  
  PLA_Routine_stack_pop( routine_name );

  if ( strcmp( routine_name, "PLA_Gemm_x_exit" ) != 0 )
    PLA_Warning( "PLA_Gemm_x_exit: history stack corrupted" );

  PLA_Routine_stack_pop( routine_name );

  if ( strcmp( routine_name, "PLA_Gemm_x" ) != 0 )
    PLA_Warning( "PLA_Gemm_x: history stack corrupted" );
